...
Use of AI: During the last day when I was trying to get physics working I was dealing a handful of different bugs (errors with collision detection and occurrences such as the player cube sinking within other entities) and I got a feeling of desperation when I had these bugs and little time left for the hand in, so I ended up using githubs agent to help fix it. 
It essentially didnt change anything of value and I felt bad having used it. 
Within about 5 minutes of using that code I scrapped it and decided to fix it without it. 
With that said, I still take responsibility for my misuse. 
The error was also a simple one, in my ResolveCollision function in physicsManager.cpp I was setting IDs for the different types of collision, in this case Obstacle and Platform. I had set the first line to have ID1 when it was supposed to be ID0. This caused a miscalculation which resulted in sinking entities. 
